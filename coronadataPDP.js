...
const coronadataPDP = async (startCreateDt, endCreateDt, callback) => {
  // 공공데이터 포털 정보
  ...
  await Promise.all(
    ...
    .then((responses) => {
      // console.log(responses.map(i => i.data.response));
      const fetchData = () => {
        const coronastatus = [];
        const usedFunctionList = [utils.makeDataLocal, utils.makeDataAgeSex];

        let dataTotalRaw = utils.makeDataTotal(
        ...
        for (var i = 0; i < dataTotalRaw.length - 1; i++) {
          ...
          nonDeathCnt.push(
            dataTotalRaw[i].decideCnt - dataTotalRaw[i].deathCnt
          );
        }
